---
layout: post
title: 【译】数据结构中你需要知道的关于树的一切（java版）
categories: 算法
description: 算法
keywords: 算法
---
...
1. 首先用add方法将根节点添加到队列中。
2. 当队列不为空时迭代。
3. 获取队列中的第一个节点，然后输出其值
4. 将左节点和右节点添加到队列
5. 在队列的帮助下我们将每一个节点值一层层输出
...
2. 如果新节点的值大于当前节点，则转到右节点。如果当前节点没有右节点，则在那里插入新节点，否则返回步骤1。
3. 如果新节点的值小于当前节点，则转到左节点。如果当前节点没有左节点，则在那里插入新节点，否则返回步骤1。
4. 这里我们没有处理特殊情况。当新节点的值等于节点的当前值时，使用规则3。考虑在子节点的左侧插入相等的值。
